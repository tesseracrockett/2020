...
```{r}
df <- lapply(ids, occ_data) 
```

This is a vector or dataframes and metadata. Each individual query made it's own dataframe. For simplicity, lets combine them into one dataframe object.


```{r}
#Combine the resultant dataframes into one large dataframe
mega_df <- df$Martialis$data  %>%
  select(scientificName, decimalLatitude, decimalLongitude) %>%
  drop_na()
for (ant in 1:length(ants)){
  print(ant)
  just_data <- df[[ant]]$data %>%
  select(scientificName, decimalLatitude, decimalLongitude) %>%
  drop_na()
  mega_df <- rbind(mega_df, just_data)
}
```
...
```{r}
a_couple_ants <- c("Martialis_heureka", "Atta_mexicana", "Atta_cephalotes", "Atta_sexdens", "Atta_vollenweideri")
subset_data <- no_bold %>% 
...
plot(obj,direction="rightwards",colors=cols,cex.points=c(0,1.5),
    pts=FALSE)
```